...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (msg.type === 'createParty') {
      ...
      room.partyOnly = true;
      parties.set(code, { leaderId: clientId, roomId: room.id, mode, withBots, memberIds: [] });
      info.partyCode = code;
      ws.send(JSON.stringify({ type:'partyCreated', code, mode, withBots }));
      console.log(`[PARTY] Created ${code} mode:${mode}`);
    }
    ...
    if (msg.type === 'joinParty') {
      ...
      if (!party) { ws.send(JSON.stringify({ type:'partyError', msg:'Invalid code.' })); return; }
      const room = rooms.get(party.roomId);
      if (!room || room.over) { ws.send(JSON.stringify({ type:'partyError', msg:'Party already ended.' })); return; }
      if (party.memberIds.length >= MAX_ROOM - 1) { ws.send(JSON.stringify({ type:'partyError', msg:'Party is full.' })); return; }
      const name = (msg.name || 'Player').slice(0, 20);
      party.memberIds.push({ clientId, name });
      info.partyCode = code;
      ws.send(JSON.stringify({ type:'partyJoined', code, mode: party.mode, withBots: party.withBots }));
      // Notify leader
      for (const [lws, linfo] of clients) {
        if (linfo.id === party.leaderId && lws.readyState === 1) {
          lws.send(JSON.stringify({ type:'partyUpdate', count: 1 + party.memberIds.length, friendName: name }));
          break;
        }
      }
    ...
    if (msg.type === 'startParty') {
      ...
      if (!party || party.leaderId !== clientId) return;
      const room = rooms.get(party.roomId);
      if (!room) return;
      const name = (msg.name || 'Leader').slice(0, 20);
      // Add leader
      const lx = 200 + Math.random() * (WORLD.W - 400);
      const ly = 200 + Math.random() * (WORLD.H - 400);
      room.players.set(clientId, defaultPlayer(clientId, name, lx, ly, room.mode));
      info.roomId = room.id;
      // Add each member, find their ws, send gameStart
      party.memberIds.forEach(member => {
        const mx = 200 + Math.random() * (WORLD.W - 400);
        const my = 200 + Math.random() * (WORLD.H - 400);
        room.players.set(member.clientId, defaultPlayer(member.clientId, member.name, mx, my, room.mode));
        for (const [mws, minfo] of clients) {
          if (minfo.id === member.clientId) {
            minfo.roomId = room.id;
            if (mws.readyState === 1) {
              mws.send(JSON.stringify({
                type:'gameStart', playerId: member.clientId,
                roomId: room.id, mode: room.mode, winGoal: room.winGoal,
                worldW: WORLD.W, worldH: WORLD.H, withBots: room.withBots
              }));
            }
            break;
          }
      ...
      // Send leader gameStart too
      ws.send(JSON.stringify({
        type:'gameStart', playerId: clientId,
        roomId: room.id, mode: room.mode, winGoal: room.winGoal,
        worldW: WORLD.W, worldH: WORLD.H, withBots: room.withBots
      }));
      console.log(`[PARTY] started ${info.partyCode} — ${room.players.size} players`);
    }
