...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (msg.type === 'createParty') {
      ...
      info.partyCode = code;
      // Don't add leader to room yet — they join when they hit PLAY
    ...
    if (msg.type === 'joinParty') {
      ...
      if ((party.members?.length || 0) >= MAX_ROOM - 1) { ws.send(JSON.stringify({ type:'partyError', msg:'Party is full.' })); return; }
      // Store member info but don't enter game yet — wait for leader to hit PLAY
      if (!party.members) party.members = [];
      const name = (msg.name || 'Player').slice(0, 20);
      party.members.push({ clientId, name, ws });
      info.partyCode = code;
      // Tell friend to wait in lobby
      ws.send(JSON.stringify({ type:'partyJoined', code, mode: party.mode, withBots: party.withBots, waitingForLeader: true }));
      // Tell leader someone joined
      broadcast(room, { type:'partyUpdate', count: 1 + party.members.length, max: MAX_ROOM });
      // Also notify leader's ws directly (they may not be in room yet)
      for (const [leaderWs, leaderInfo] of clients) {
        if (leaderInfo.partyCode === code && leaderWs !== ws) {
          leaderWs.send(JSON.stringify({ type:'partyUpdate', count: 1 + party.members.length, max: MAX_ROOM, friendName: name }));
        }
      }
      console.log(`[PARTY] ${name} joined party ${code} — waiting for leader`);
    }

    // ── START PARTY (leader hits PLAY) ───────────────────────
    if (msg.type === 'startParty') {
      const party = parties.get(info.partyCode);
      if (!party) return;
      const room  = rooms.get(party.roomId);
      if (!room)  return;
      const name  = (msg.name || 'Leader').slice(0, 20);
      // Add leader to room
      if (!info.roomId) {
        const startX = 200 + Math.random() * (WORLD.W - 400);
        const startY = 200 + Math.random() * (WORLD.H - 400);
        room.players.set(clientId, defaultPlayer(clientId, name, startX, startY, room.mode));
        info.roomId = room.id;
      }
      // Add all waiting members to room and send them gameStart
      if (party.members) {
        party.members.forEach(member => {
          const memberInfo = [...clients.entries()].find(([,i]) => i.id === member.clientId || member.clientId === [...clients.entries()].find(([w]) => w === member.ws)?.[1]?.id);
          // Find by ws reference
          const mInfo = clients.get(member.ws);
          if (mInfo) mInfo.roomId = room.id;
          const sx = 200 + Math.random() * (WORLD.W - 400);
          const sy = 200 + Math.random() * (WORLD.H - 400);
          room.players.set(member.clientId, defaultPlayer(member.clientId, member.name, sx, sy, room.mode));
          // Tell friend to start game
          if (member.ws.readyState === 1) {
            member.ws.send(JSON.stringify({
              type: 'gameStart',
              playerId: member.clientId,
              roomId:   room.id,
              mode:     room.mode,
              winGoal:  room.winGoal,
              worldW:   WORLD.W,
      ...
      // Tell leader they've joined
      ws.send(JSON.stringify({ type:'joined', playerId:clientId, roomId:room.id, mode:room.mode, winGoal:room.winGoal, worldW:WORLD.W, worldH:WORLD.H, withBots:room.withBots }));
      console.log(`[PARTY] ${name} started party ${info.partyCode} — ${room.players.size} players`);
    }

    // ── JOIN GAME (public matchmaking) ────────────────────────
    if (msg.type === 'join') {
      ...
      const startX   = 200 + Math.random() * (WORLD.W - 400);
      const startY   = 200 + Math.random() * (WORLD.H - 400);
      room.players.set(clientId, defaultPlayer(clientId, name, startX, startY, mode));
      info.roomId = room.id;
